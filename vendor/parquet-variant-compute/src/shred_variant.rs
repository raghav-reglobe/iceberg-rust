...
pub(crate) fn make_variant_to_shredded_variant_arrow_row_builder<'a>(
    ...
    let builder = match data_type {
        ...
        => {
            ...
            // A shredded value must reproduce the original: numerics that
            // would round at the typed_value's scale stay variant values.
            builder.require_exact_numeric();
...
mod tests {
    ...
    /// Shredding never rounds: a decimal with more fractional digits than
    /// the target scale, or a double the decimal cannot reproduce, is not
    /// shredded — it stays in `value` and reads back unchanged. Exactly
    /// representable values (same scale, dropped digits all zero, integers
    /// that fit) shred as before.
    #[test]
    fn inexact_decimals_are_not_shredded() {
        use arrow::array::Decimal128Array;
        use parquet_variant::{VariantDecimal4, VariantDecimal8};

        let values: Vec<Variant<'static, 'static>> = vec![
            Variant::from(VariantDecimal4::try_new(68, 2).unwrap()), // 0.68 -> stays a value
            Variant::from(VariantDecimal4::try_new(7, 1).unwrap()),  // 0.7  -> typed 7
            Variant::from(VariantDecimal4::try_new(70, 2).unwrap()), // 0.70 -> typed 7 (exact)
            Variant::from(VariantDecimal8::try_new(996, 3).unwrap()), // 0.996 -> stays a value
            Variant::Double(0.68),                                   // -> stays a value
            Variant::Double(0.5),                                    // -> typed 5 (exact)
            Variant::Int32(3),                                       // -> typed 30
        ];
        ...
        let shredded = shred_variant(&array, &DataType::Decimal128(9, 1)).unwrap();
        ...
        let typed = shredded
            ...
            .unwrap();
        let got: Vec<Option<i128>> = (0..values.len())
            .map(|i| typed.is_valid(i).then(|| typed.value(i)))
            .collect();
        assert_eq!(
            got,
            vec![None, Some(7), Some(7), None, None, Some(5), Some(30)]
        );
        // Not-shredded rows keep their exact value; shredded rows have none.
    ...
    /// A plain cast (not shredding) keeps its rounding semantics.
    #[test]
    fn plain_casts_still_round() {
        use crate::type_conversion::{variant_to_unscaled_decimal, variant_to_unscaled_decimal_exact};
        use arrow::datatypes::Decimal128Type;
        use parquet_variant::VariantDecimal4;

        let v = Variant::from(VariantDecimal4::try_new(68, 2).unwrap());
        assert_eq!(variant_to_unscaled_decimal::<Decimal128Type>(&v, 9, 1), Some(7));
        assert_eq!(variant_to_unscaled_decimal_exact::<Decimal128Type>(&v, 9, 1), None);
        assert_eq!(variant_to_unscaled_decimal_exact::<Decimal128Type>(&v, 9, 2), Some(68));
        assert_eq!(variant_to_unscaled_decimal_exact::<Decimal128Type>(&v, 9, 3), Some(680));
        assert_eq!(
            variant_to_unscaled_decimal_exact::<Decimal128Type>(&Variant::Int64(120), 9, -1),
            Some(12),
            "trailing zeros drop exactly"
        );
        assert_eq!(
            variant_to_unscaled_decimal_exact::<Decimal128Type>(&Variant::Int64(123), 9, -1),
            None
        );
        assert_eq!(
            variant_to_unscaled_decimal_exact::<Decimal128Type>(&Variant::ShortString("0.7".try_into().unwrap()), 9, 1),
            None,
