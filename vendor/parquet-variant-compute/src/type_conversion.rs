...
/// Like [`variant_to_unscaled_decimal`], but only when the value is
/// representable EXACTLY at (`precision`, `scale`); anything that would
/// round returns `None`.
///
/// - Decimal variants (`Decimal4/8/16`) and integer variants (scale 0)
///   rescale only when every dropped fractional digit is zero (up-scaling
///   is always exact); the result must fit `precision`
/// - Floating point variants (`Float/Double`) convert only when the
///   decimal reproduces the same `f64`
/// - String variants are not numbers: `None`
///
/// Shredding relies on this: a shredded value must reproduce the original
/// exactly, so a value that cannot be represented in `typed_value` stays a
/// variant `value` instead of being rounded into the typed column.
pub(crate) fn variant_to_unscaled_decimal_exact<O>(
    variant: &Variant<'_, '_>,
    precision: u8,
    scale: i8,
) -> Option<O::Native>
where
    ...
    O::Native: DecimalCast,
{
    fn dropped_digits_are_zero(unscaled: i128, from_scale: i8, to_scale: i8) -> bool {
        if from_scale <= to_scale {
            return true;
        }
        let dropped = (from_scale - to_scale) as u32;
        // i128 holds 10^38; a decimal variant's scale never exceeds that.
        match 10_i128.checked_pow(dropped) {
            Some(divisor) => unscaled % divisor == 0,
            None => unscaled == 0,
        }
    }
    ...
    match variant {
        ...
        | Variant::Decimal8(_)
        | Variant::Decimal16(_) => {
            let (unscaled, from_scale) = match variant {
                Variant::Int8(i) => (i128::from(*i), 0),
                Variant::Int16(i) => (i128::from(*i), 0),
                Variant::Int32(i) => (i128::from(*i), 0),
                Variant::Int64(i) => (i128::from(*i), 0),
                Variant::Decimal4(d) => (i128::from(d.integer()), d.scale() as i8),
                Variant::Decimal8(d) => (i128::from(d.integer()), d.scale() as i8),
                Variant::Decimal16(d) => (d.integer(), d.scale() as i8),
                _ => unreachable!(),
            };
            if !dropped_digits_are_zero(unscaled, from_scale, scale) {
                return None;
            }
            variant_to_unscaled_decimal::<O>(variant, precision, scale)
        }
        Variant::Float(_) | Variant::Double(_) => {
            let f = match variant {
                Variant::Float(f) => f64::from(*f),
                Variant::Double(f) => *f,
                _ => unreachable!(),
            };
            let scaled = variant_to_unscaled_decimal::<O>(variant, precision, scale)?;
            // Exact iff the decimal reads back as the same double.
            let back = scaled.to_i128()? as f64 / 10_f64.powi(scale as i32);
            (back == f).then_some(scaled)
        }
        _ => None,
    }
}
