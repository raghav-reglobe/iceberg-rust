...
impl<'a> PrimitiveVariantToArrowRowBuilder<'a> {
    /// Numeric conversions must be EXACT: a decimal that would round at the
    /// target scale, or a float the decimal cannot reproduce, is reported as
    /// not converted instead of being rounded. Shredding requires this — a
    /// shredded value has to reproduce the original, so such values stay
    /// variant `value`s. A plain cast keeps its rounding semantics.
    pub fn require_exact_numeric(&mut self) {
        use PrimitiveVariantToArrowRowBuilder::*;
        match self {
            Decimal32(b) => b.exact = true,
    ...
    T::Native: DecimalCast,
{
    ...
    precision: u8,
    scale: i8,
    /// Convert only values representable exactly at (`precision`, `scale`);
    /// a value that would round is reported as not converted. Shredding
    /// sets this — see [`PrimitiveVariantToArrowRowBuilder::require_exact_numeric`].
    exact: bool,
}
    ...
    T::Native: DecimalCast,
{
    ...
    fn append_value(&mut self, value: &Variant<'_, '_>) -> Result<bool> {
        ...
        match variant_cast_with_options(value, self.cast_options, |value| {
            if exact {
                variant_to_unscaled_decimal_exact::<T>(value, self.precision, self.scale)
            } else {
